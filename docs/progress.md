...
| 主参考作品 | 2024 Phoenix |
| 当前目标 | 将用户地址空间接入任务结构，为按任务切换页表做准备 |
| 当前完成度 | 已完成最小启动、trap、syscall、两任务轮转、物理页帧分配、Sv39 页表基础、区间映射、内核地址空间结构、临时用户段权限映射、Sv39 内核分页开启和用户地址空间自检 |
...
| 4 | 任务绑定用户地址空间 | 每个任务保存自己的 `MemorySet` 并能取得 `satp` token | 下一步 |
| 5 | 按任务切换页表 | 进入用户任务前切换到对应用户页表，syscall/trap 后仍正常 | 未开始 |
| 6 | 用户程序加载 | 从内嵌用户程序或 ELF 构造用户地址空间 | 未开始 |
| 7 | 基础 syscall 兼容 | 实现 `write`、`exit`、`yield`、`getpid` 等基础 syscall | 未开始 |
| 8 | 测试矩阵 | 建立官方测例通过情况记录 | 未开始 |
