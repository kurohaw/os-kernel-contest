...
- RV/LA 同步追加 `creat*`、`symlink*`、`unlink*`、`rename*`、`renameat*`、
  `truncate*`、`utime*`、`utimensat01`、`utimes01`、`sendfile*` 和 `statx*`
  子集。`link*` 暂缓，避免硬链接多名字和删除语义把 VFS 稳定性拖垮。
- RISC-V 补齐 `statx=291`，支持普通路径和 `AT_EMPTY_PATH` 的 fd/cwd 路径，
  填充 Linux `struct statx` 的 basic stats，并处理非法 flag 与
  `STATX__RESERVED` 边界。
- RISC-V 补齐 `symlinkat=36`，通过 `FileLNK` inode 保存软链接目标；
  `readlinkat` 对真实软链接返回目标，同时保留 `/proc/self/exe` 返回
  `/lmbench_all` 的 lmbench 兼容路径。
...
- 在 vendored `heapless/build.rs` 中用 `cargo:rustc-check-cfg` 声明全部自定义
  ...
  RISC-V ELF；本机无真实 LoongArch 工具链时按设计生成 `kernel-la` 占位。

## 2026-06-28 838 回退与 BusyBox 沙箱止血

- `301e9717 feat: batch LoongArch big test probes` 的官方结果实际得分为
  `838.5995587579628`，但页面提示 `JSON格式错误`。原因是评测脚本输出 JSON
  前混入了平台侧异常：`gzip: sdcard-rv.img already exists; not overwritten`，
  导致前端解析失败；不是内核编译失败。
- 该轮真正的代码回退是 LA 侧：basic 四列仍为 `408`，但 LA BusyBox 从约
  `108` 掉到 `10`，LA Lua/libcbench 归零。日志显示 `busybox-musl` 执行
  `du` 时递归进入 `/musl/ltp/testcases/...` 大目录，随后在 LoongArch 内核中
  触发 `Unhandled PLV0 Page Fault`，后续 LA 组没有继续有效计分。
- 当前撤回 `301e9717` 中新增的 `iozone/iperf/netperf/cyclictest` 高风险入口，
  避免再次把 983 基线打回 320 或 838。
- 新增 `run_busybox_script`：将 BusyBox 二进制、脚本、命令清单和 `ls` 链接放入
  `/tmp/swtc-busybox-musl` 或 `/tmp/swtc-busybox-glibc` 后再执行，避免 `du` 和
  `find` 从 `/musl` 或 `/glibc` 根目录扫到 LTP/libctest 等大树。
- lmbench 不再使用无保护的整组脚本，而是在 basic、BusyBox、Lua、libcbench、
  libctest 和 LTP 全部结束后才执行；每条官方命令单独 timeout，失败时不应截断
  前面的 functional 得分。
- 下一次评测目标是先恢复到 983 附近，再争取 LA libctest/LTP 和后置 lmbench
  产生大于百分快速增量。
...
- 最新官方结果为 2026-06-23 11:39:59，`Accepted / 320.0`。
- 得分构成：basic=204、BusyBox=98、Lua=18；libcbench、libctest、lmbench 均为
  0。
- 该形态与 iozone-lite 的 320 回归类似：基础组仍运行，但后续性能/库测试全部丢分。
  结合本轮唯一高风险变更，判断为全局 `/bin/sh`、`/lib`、`/lib64/lp64d`、
  `/usr/lib64`、`/etc/passwd`、`/tmp/memfd` 运行环境骨架污染了 libcbench/libctest
  原本隔离的 tmpfs staging。
