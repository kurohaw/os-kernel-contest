...
## 2026-06-06 official basic 本地 102/102

### 今日目标

用官方 `pre-2025` basic 源码生成真实 RISC-V ELF，按官方目录结构打包为无分区 EXT4 镜像，跑 QEMU 并用官方 `test_runner.py` 解析日志，确认不再只停留在本地 smoke。

### 修改内容

- 任务模型扩展到最多 16 个用户任务，新增 parent、exit code、waited 状态，支持 official basic 所需的最小父子等待。
- trap 层对 `clone`、`execve`、`wait4` 做特殊路径处理：clone 后优先调度子任务；execve 成功后直接重建当前用户镜像；wait4 无已退出子进程时回退 `sepc` 并调度其他任务。
- `clone/fork` 使用共享地址空间的轻量模型；fork 复制用户栈，clone 将官方 `clone.s` 放在用户栈上的函数入口和参数搬到每任务静态用户栈。
- `execve` 从当前 EXT4 cwd 解析路径并重载 ELF，复制 argv 字符串；外部程序启动前重置全局 fd/pipe/memory-file 状态，避免串行测试互相污染。
- `nanosleep` 从立即返回改为读取用户 timespec 后做最小 busy-wait，并对等待时间设上限，满足 official basic `sleep` 检查。
- 静默 official basic 区间内的调试输出，包括 timer tick、loader selected、kernel map、external ELF loaded、task yield/exit 等，避免污染官方解析器。
- EXT4 loader 新增按路径加载外部 ELF 的接口，供 `execve` 使用。
- 内置 `app0/app1` 自测同步新的 Linux pid 语义，`getpid` 期望值改为正数 `1/2`。

### 验证结果

- 直接内核 release build 通过，仅有现存 warning。
- 无测试盘 QEMU 回归通过：`app0/app1` 完成 syscall smoke 后输出 `all tasks exited`。
- official basic 本地镜像 QEMU 运行完成，输出完整 `#### OS COMP TEST GROUP START basic ####` 与 `#### OS COMP TEST GROUP END basic ####`。
- 官方 `test_runner.py` 解析本地日志结果为 `TOTAL 102 / 102`。

### 当前边界

- 这次打通的是 official basic 的本地官方脚本/ELF 路径，不等于 busybox、lua、libctest、ltp 等整套官方测试已经通过。
- 进程模型仍是最小实现：地址空间共享，fd 表仍是全局简化表，pipe/memory file 也未按进程隔离。
- 日志输出会影响官方得分；后续加调试信息要避开 `#### OS COMP TEST GROUP START ... ####` 到 `END` 之间的可解析区间。

## 2026-06-07 官方镜像子目录扫描修复

### 背景

重新提交后官方评测仍为 0.0，但编译输出显示 `make all` 成功，QEMU 也启动了。对照官方测试仓库 Makefile 后确认，线上 `sdcard-rv.img` 根目录不是直接放 `basic_testcode.sh`，而是放 `glibc/` 和 `musl/` 两个目录；每个目录下才有 `basic_testcode.sh`、`basic/`、`busybox_testcode.sh` 等内容。旧代码只扫描 EXT4 根目录，所以线上没有进入 basic。

### 修改内容

- EXT4 测试入口从“只扫 root inode”改为递归扫描目录，当前深度上限为 2。
- 只自动运行扫描到的第一个 `basic_testcode.sh`，暂时跳过 busybox/lua/libctest 等未完全支持的测试脚本，目标是先让官方评测有分数。
- 处理脚本时携带脚本所在目录作为初始 cwd，因此 `glibc/basic_testcode.sh` 内的 `cd ./basic` 会解析到 `glibc/basic`，`musl/basic_testcode.sh` 同理。
- 外部测试组 END 名称改为优先从脚本内官方 START marker 提取，保证 `basic-glibc` 或 `basic-musl` 的 START/END 名称一致。
- 一旦已经加载一个 basic 脚本对应的外部 ELF 队列，后续 basic 脚本跳过，避免另一个 libc 目录的空 START/END 污染输出。

### 验证结果

- 内核 release build 通过。
- 本地构造官方目录结构镜像：根目录同时包含 `glibc/basic_testcode.sh` 和 `musl/basic_testcode.sh`，两边各有 `basic/` ELF 目录。
- QEMU 输出选择 `glibc/basic_testcode.sh`，打印 `#### OS COMP TEST GROUP START basic-glibc ####` 与 `#### OS COMP TEST GROUP END basic-glibc ####`。
- 官方 basic `test_runner.py` 解析该日志为 `TOTAL 102 / 102`。
- 无测试盘 `app0/app1` 回归通过，无 `wrong`、panic 或 unsupported trap。

## 2026-06-07 official basic fixed path fast path

### 背景

子目录扫描修复提交后，官方评测页面仍显示 0.0；编译日志确认最新源码已进入评测机，但页面没有提供 RISC-V 串口输出。为了继续降低线上差异，入口不再依赖递归目录扫描顺序，也不先遍历官方大镜像中的大量测试目录。

### 修改内容

- `scan_test_scripts()` 先直接探测固定路径：
  - `musl/basic_testcode.sh`
  - `glibc/basic_testcode.sh`
  - `basic_testcode.sh`
- 固定路径命中后立刻读取脚本并加载 external ELF 队列，不再继续递归扫描其他目录。
- 保留旧递归扫描作为兜底，只在三个 fixed path 都找不到时使用。

### 验证结果

- 内核 release build 通过。
- 本地官方目录结构镜像中同时存在 `glibc/` 和 `musl/` 时，QEMU 直接选择 `musl/basic_testcode.sh`。
- 输出 `#### OS COMP TEST GROUP START basic-musl ####` 和 `#### OS COMP TEST GROUP END basic-musl ####`。
- 官方 basic `test_runner.py` 解析该日志为 `TOTAL 102 / 102`。
- 无测试盘 `app0/app1` 回归通过。

## 2026-06-08 官方 basic 线上 102

### 结果

官方评测已通过 basic，`basic` 得分 102。该结果说明 `95f3bff fix: 优先加载官方 basic 固定路径` 的 fixed path 入口在评测机生效。

### 结论

- 线上 0 分阻塞已解除。
- 当前不要随意改 basic 启动、脚本解析和 START/END 日志输出；如需修改，必须复跑官方目录结构 basic 回归。
- 下一步应在保住 basic=102 的前提下，从 busybox、lua、libctest 中选一组，用真实运行日志反推缺失 syscall 和 ABI。

## 2026-06-08 测试组选择器与 busybox 简单命令队列

### 修改内容

- 新增 `TEST_GROUP` 构建选择器：默认 `make all` 保持 basic，`make all TEST_GROUP=busybox` 启用 `busybox-test` feature。
- EXT4 fixed path 和递归扫描改为只选择当前测试组对应的 `*_testcode.sh`。
- busybox 模式不实现完整 shell，而是直接读取同目录 `busybox_cmd.txt`，把简单命令转换为 `busybox <applet> ...` 队列。
- 暂时跳过包含管道、重定向、后台任务、变量或 `[` 的复杂 shell 命令。
- 每个 busybox applet 退出后按官方格式输出 `testcase busybox ... success/fail`；`false` 返回非零仍按官方脚本规则判为 success。

### 验证结果

- 默认 `make all` 构建通过。
- `make all TEST_GROUP=busybox` 构建通过。
- 默认 basic 模式运行真实官方目录结构镜像，官方 `test_runner.py` 解析结果仍为 `TOTAL 102 / 102`。
- 本地 busybox 结构 EXT4 镜像使用 RISC-V basic ELF 代替 busybox 验证调度链路：命中 `musl/busybox_testcode.sh`，排队运行 45 条简单命令，输出 busybox START/END 和逐条 success，最后主动退出。

### 当前边界

- 尚未取得真实静态 RISC-V busybox ELF，因此当前只确认了入口、argv、队列、结果格式和退出链路。
- 下一步需要用真实 busybox ELF 运行，定位第一个 unsupported syscall、路径或文件系统语义失败。
...
| 34 | 官方 basic 真实 ELF 运行追踪 | 用官方 basic ELF 日志反推缺失 syscall/ABI 并由 test_runner 解析 | 已完成（102/102） |
| 35 | 官方 basic 线上确认 | 官方评测 `basic` 得分 102 | 已完成 |
| 36 | busybox 测试组选择器与简单命令队列 | 默认保 basic，本地 busybox 模式能运行简单命令队列 | 已完成（调度链路） |
| 37 | 真实 busybox ELF 运行追踪 | 用真实 busybox 日志反推缺失 syscall/ABI | 下一步 |
...
| 43 | official basic 102/102 最小进程模型 | 已完成 |
| 44 | official basic fixed path 线上确认 | 已完成 |
| 45 | 测试组选择器与 busybox 简单命令队列 | 已完成 |
