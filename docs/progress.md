...
| 主参考作品 | 2024 Phoenix |
| 当前目标 | 设计用户地址空间，为用户程序加载做准备 |
| 当前完成度 | 已完成最小启动、trap、syscall、两任务轮转、物理页帧分配、Sv39 页表基础、区间映射、内核地址空间结构、临时用户段权限映射和 Sv39 内核分页开启 |
...
| 3 | 设计用户地址空间 | 用户入口和用户栈通过页表映射后仍能 syscall | 下一步 |
