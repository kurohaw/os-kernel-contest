...
| 主参考作品 | 2024 Phoenix |
| 当前目标 | 建立内核地址空间结构，为后续开启分页做准备 |
| 当前完成度 | 已完成最小启动、trap、syscall、两任务轮转、物理页帧分配、Sv39 页表基础和区间映射 |
...
| 顺序 | 任务 | 完成标准 | 状态 |
|---|---|---|---|
| 1 | 设计内核地址空间结构 | `MemorySet::new_kernel()` 能完成内核段恒等映射自检 | 未开始 |
| 2 | 开启内核分页 | 写入 `satp` 后内核仍能启动、打印、处理中断 | 未开始 |
