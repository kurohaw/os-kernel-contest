...
| 当前修复方向 | 冻结 musl libctest full-static，锁定 484 基线后转向 lmbench-lite 或 libcbench 单指标探针 |
...
或 `C` 队列协议。下一次官方评测的核心要求是保持 basic、BusyBox、Lua、
libcbench 与 107 个 libctest case，不因新指标探针回退。
...
- 新指标探针若失败，必须只表现为单命令 `FAIL` 或 timeout，不能引发 kernel panic。
...
1. 下一次先确认 484 基线不回退；若低于 480，优先撤回新增的新指标探针。
2. `libctest` 已满分，除非官方回归，不再修改 allowlist、timeout 或 `C` 队列协议。
3. 回到 `lmbench-lite`，逐步加入 `lat_select file`、
