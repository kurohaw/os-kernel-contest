...
| 证据 | 结论 |
|---|---|
| 最新可见官方结果 | 2026-06-20 17:52:16，`Accepted / 392.9002806839883` |
| 最新稳定结果 | 2026-06-20 17:52:16，`Accepted / 392.9002806839883`；libctest-musl 已进 16 分 |
...
| 上一条通过基线 | 2026-06-20 17:52:16，`Accepted / 392.9002806839883` |
| 通过基线得分构成 | RISC-V basic `204`、BusyBox `98`、Lua `18`、libcbench `56.9002806839883`、libctest `16` |
| 上一条编译错误 | 2026-06-19 19:09:49，`Compile Error / 0.00`；`no matching package found: ahash`，本轮通过移除 `hashbrown` 依赖链修复 |
| 上一条高分结果 | 2026-06-20 17:52:16，`Accepted / 392.9002806839883` |
| 最新线上得分 | basic `204`、BusyBox `98`、Lua `18`、libcbench `56.9002806839883`、libctest `16` |
| 当前修复方向 | 在 392 基线上只把 musl libctest 从 16 个稳定 case 扩到 20 个，验证 4 个低风险静态 case |
| 本轮代码基线 | 已基于 GitHub/GitLab `main` 的 `154ce46 feat: extend musl libctest to 16 cases` |
...
这轮在 17:52 已恢复的 392 基线上继续小步提分：不恢复 `4602678` 的
64-case libctest 扩容，只新增 `strtod_simple`、`strtof`、`udiv`、`wcsstr`
四个低风险静态 case。下一次官方评测的核心要求是保持
basic、BusyBox、Lua、libcbench 与现有 16 个 libctest case，不因 4-case 探针回退。
...
7. `C` 队列记录只服务 musl libctest，且本轮只从 16 个稳定 case 扩到 20 个 case。
...
- `libctest-musl` 至少保持 16 分；理想情况新增 1 到 4 分。
- 新增 `strtod_simple`、`strtof`、`udiv`、`wcsstr`
...
1. 下一次先确认 392 基线不回退；若低于 389，优先回退本次 4-case 探针。
2. 若 libctest 变为 17 到 20 分且无 panic，再继续按 4 到 8 个 case 一批扩容。
