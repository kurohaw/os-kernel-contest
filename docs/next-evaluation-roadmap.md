...
| 证据 | 结论 |
|---|---|
| 最新可见官方结果 | 2026-06-21 11:49:38，`Accepted / 412.92336789756513` |
| 最新稳定结果 | 2026-06-21 11:49:38，`Accepted / 412.92336789756513`；libctest-musl 已进 36 分 |
...
| 上一条通过基线 | 2026-06-21 11:49:38，`Accepted / 412.92336789756513` |
| 通过基线得分构成 | RISC-V basic `204`、BusyBox `98`、Lua `18`、libcbench `56.92336789756515`、libctest `36` |
| 上一条编译错误 | 2026-06-19 19:09:49，`Compile Error / 0.00`；`no matching package found: ahash`，本轮通过移除 `hashbrown` 依赖链修复 |
| 上一条高分结果 | 2026-06-21 11:49:38，`Accepted / 412.92336789756513` |
| 最新线上得分 | basic `204`、BusyBox `98`、Lua `18`、libcbench `56.92336789756515`、libctest `36` |
| 当前修复方向 | 在 412 基线上只把 musl libctest 从 36 个稳定 case 扩到 107 个 static case，目标拿满 musl static libctest |
| 本轮代码基线 | 已基于 GitHub/GitLab `main` 的 `883811f feat: extend musl libctest to 36 cases` |
...
这轮在 11:49 已恢复的 412 基线上直接推进 musl libctest static 全量：不改
lmbench、iozone、ltp、网络或 LoongArch，只把 allowlist 扩到官方
`libc-test/static.txt` 的 107 个 case。下一次官方评测的核心要求是保持
basic、BusyBox、Lua、libcbench 与现有 36 个 libctest case，不因 full-static
探针回退。
...
7. `C` 队列记录只服务 musl libctest，且本轮只从 36 个稳定 case 扩到 107 个 static case。
...
- `libctest-musl` 至少保持 36 分；理想情况直接接近或达到 static 满分。
- 新增到 107 个 static case
...
1. 下一次先确认 412 基线不回退；若低于 400，优先回退本次 full-static 探针。
2. 若 libctest 未满但无 panic，再根据失败日志把未通过 case 分组隔离或补 syscall。
