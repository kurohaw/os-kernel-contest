...
| 证据 | 结论 |
|---|---|
| 最新可见官方结果 | 2026-06-20 21:34:30，`Accepted / 400.50066694574866` |
| 最新稳定结果 | 2026-06-20 21:34:30，`Accepted / 400.50066694574866`；libctest-musl 已进 24 分 |
...
| 上一条通过基线 | 2026-06-20 21:34:30，`Accepted / 400.50066694574866` |
| 通过基线得分构成 | RISC-V basic `204`、BusyBox `98`、Lua `18`、libcbench `56.50066694574863`、libctest `24` |
| 上一条编译错误 | 2026-06-19 19:09:49，`Compile Error / 0.00`；`no matching package found: ahash`，本轮通过移除 `hashbrown` 依赖链修复 |
| 上一条高分结果 | 2026-06-20 21:34:30，`Accepted / 400.50066694574866` |
| 最新线上得分 | basic `204`、BusyBox `98`、Lua `18`、libcbench `56.50066694574863`、libctest `24` |
| 当前修复方向 | 在 400 基线上只把 musl libctest 从 24 个稳定 case 扩到 36 个，验证 12 个较低风险静态 case |
| 本轮代码基线 | 已基于 GitHub/GitLab `main` 的 `caa880d feat: extend musl libctest to 24 cases` |
...
这轮在 21:34 已恢复的 400 基线上加快 libctest 提分：不恢复 `4602678` 的
64-case libctest 扩容，只新增 12 个较低风险静态 case：
`clocale_mbfuncs`、`clock_gettime`、`fnmatch`、`inet_pton`、`mbc`、`setjmp`、
`sscanf`、`sscanf_long`、`strftime`、`strtod`、`strtold`、`swprintf`。下一次
官方评测的核心要求是保持 basic、BusyBox、Lua、libcbench 与现有 24 个 libctest
case，不因 12-case 探针回退。
...
7. `C` 队列记录只服务 musl libctest，且本轮只从 24 个稳定 case 扩到 36 个 case。
...
- `libctest-musl` 至少保持 24 分；理想情况新增 4 到 12 分。
- 新增 12 个 case
...
1. 下一次先确认 400 基线不回退；若低于 397，优先回退本次 12-case 探针。
2. 若 libctest 变为 29 到 36 分且无 panic，再继续按 8 到 12 个 case 一批扩容。
