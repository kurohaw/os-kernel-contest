...
| 证据 | 结论 |
|---|---|
| 最新可见官方结果 | 2026-06-24 13:40:19，`Accepted / 320.0`；basic、BusyBox、Lua 保持，libcbench/libctest/cyclictest 为 0 |
...
| 当前修复方向 | 撤回 cyclictest 的扫描、暂存和 `K` 队列协议，先恢复 480 基线 |
...
2. **LTP 批量队列**：下一步读取官方 `ltp_testcode.sh`，先接入无需网络、挂载和
   特权设备的低耦合批次；必须保留单项 timeout 和串行回收。
3. **LoongArch**：独立实现真实 `kernel-la`，不再使用 RISC-V 占位 ELF；该路线
   潜在增量最大，但构建、启动和驱动适配成本也最高。
...
9. `C/A` 队列记录服务 musl libctest；107 个 static case 已线上通过，110 个
...
2. dynamic libctest 线上确认后冻结该组，转向 LTP 无网络低耦合批次。
