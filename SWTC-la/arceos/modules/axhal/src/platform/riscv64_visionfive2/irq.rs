...
pub fn register_handler(scause: usize, handler: IrqHandler) -> bool {
    debug!("enter register_handler");
    with_cause!(
        scause,
        @TIMER => if !TIMER_HANDLER.is_inited() {
            debug!("run TIMER_HANDLER.init_once(handler);");
            TIMER_HANDLER.init_once(handler);
            true
        } else {
            debug!("not run TIMER_HANDLER.init_once(handler);");
            false
        },
...
pub fn dispatch_irq(scause: usize) {
    with_cause!(
        ...
        @EXT => {
            // TODO: Get actual external IRQ number from PLIC
            // For now, use the scause value without the interrupt bit
            let ext_irq_num = scause & !INTC_IRQ_BASE;
            crate::irq::dispatch_irq_common(ext_irq_num);
        },
    );
}
