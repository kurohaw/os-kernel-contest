...
- lmbench 保留 9-command lite 探针，并保留 `/lmbench_all`、`lat_sig`、
  ...
  当前已撤回。
- lmbench 暂停继续投入；当前改为 LTP 小批量探针，暂存 glibc/musl
  `ltp/testcases/bin` 中 12 个基础 syscall case，并通过 `L` 队列逐个串行执行。
...
最新可见结果为 2026-06-23 用户截图：编译状态 `Accepted`，总分
`483.94872319405437`。其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为
`18`，libcbench 为 `56.94872319405434`，musl libctest 为 `107`，lmbench 仍为
`0`。这说明撤回 `d6746eb` 的全局运行环境骨架后，483-484 基线已经恢复。
...
上一条回归结果为 2026-06-23 11:39:59：编译状态 `Accepted`，总分 `320.0`。
该结果来自 `d6746eb` 的全局运行环境骨架，basic、BusyBox、Lua 保持得分，但
...
下一轮重点验证两件事：先看撤回运行环境骨架后是否恢复 480 基线，再看 LTP 小批量
是否从 0 变成有分。lmbench 暂停，不再用全局 `/lib`、`/bin/sh` 或 `/etc/passwd`
盲试。
