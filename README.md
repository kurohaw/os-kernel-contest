...
- musl `libctest` 小批量探针已线上验证 24 个稳定 case；当前只识别官方
  `/musl/libctest_testcode.sh`，读取 `run-static.sh`，暂存
  `entry-static.exe` 和可选 `runtest.exe`。当前 24 个 case 已线上通过，本轮从
  24 扩到 36 个，新增 `clocale_mbfuncs`、`clock_gettime`、`fnmatch`、
  `inet_pton`、`mbc`、`setjmp`、`sscanf`、`sscanf_long`、`strftime`、`strtod`、
  `strtold`、`swprintf`。
...
最近可见高分结果为 2026-06-20 21:34:30：编译状态 `Accepted`，总分
`400.50066694574866`。其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为
`18`，libcbench 为 `56.50066694574863`，musl libctest 为 `24`。本轮继续保持
单指标策略，不扩大 lmbench、iozone、ltp、网络或 LoongArch，只把 musl libctest
从 24 个稳定 case 扩到 36 个 case。
