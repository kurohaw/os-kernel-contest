...
最新可见结果为 2026-06-23 11:39:59：编译状态 `Accepted`，总分 `320.0`。
其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为 `18`，但 libcbench 和
libctest 均回到 `0`。这说明 `d6746eb` 的全局运行环境骨架污染了后续测试环境，
当前已撤回该改动。
...
下一轮重点先验证撤回运行环境骨架后是否恢复 480 基线；恢复后再继续研究 lmbench
真实串口输出，不再用全局 `/lib`、`/bin/sh` 或 `/etc/passwd` 盲试。
