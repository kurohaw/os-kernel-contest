...
- lmbench 仍保持 9-command lite 范围；本轮不新增命令，只补齐
  `readlinkat(/proc/self/exe)` 当前兼容返回 `/lmbench_all` 所需的 tmpfs 根路径
  别名，并用 fake lmbench EXT4 盘验证 glibc/musl 脚本识别、队列执行和主动关机。
...
后续提分优先看 lmbench-lite 的真实官方输出：如果仍为 0，再根据是否出现
`Simple syscall:`、`Select on 100 fd` 或 `Signal handler installation:` 来决定
继续修执行路径还是 parser 分组。
