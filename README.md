...
- musl `libctest` 小批量探针已线上验证 20 个稳定 case；当前只识别官方
  `/musl/libctest_testcode.sh`，读取 `run-static.sh`，暂存
  `entry-static.exe` 和可选 `runtest.exe`。当前 20 个 case 已线上通过，本轮从
  20 小步扩到 24 个，只新增 `search_hsearch`、`search_insque`、
  `search_lsearch`、`search_tsearch`。
...
最近可见高分结果为 2026-06-20 21:09:40：编译状态 `Accepted`，总分
`397.32511382265227`。其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为
`18`，libcbench 为 `57.325113822652256`，musl libctest 为 `20`。本轮继续保持
单指标策略，不扩大 lmbench、iozone、ltp、网络或 LoongArch，只把 musl libctest
从 20 个稳定 case 小步扩到 24 个 case。
