...
- lmbench 已从 9-command lite 扩为 24-command 官方主项队列，并补齐
  `/lmbench_all`、`lat_sig`、`/var/tmp/XXX` 和 `/tmp/hello` 等兼容资源；submit
  构建默认关闭 `stack_trace`，减少 syscall/文件热路径的调试栈记录开销。
...
最新可见结果为 2026-06-22 15:40:33：编译状态 `Accepted`，总分
`483.89530518161376`。其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为
`18`，libcbench 为 `56.89530518161379`，musl libctest 为 `107`，lmbench 仍为
`0`。这说明 `/lmbench_all` 根路径别名没有破坏 483-484 基线，但 9-command
lite 尚未进分。
...
下一轮重点验证完整 lmbench 主项和关闭 `stack_trace` 后的真实效果：若总分低于
480，优先撤回 lmbench 扩容；若保持基线且 lmbench 有分，再继续优化 pipe、select
和 fork/exec 热路径。
