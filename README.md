...
最近可见高分结果为 2026-06-21 12:05:08：编译状态 `Accepted`，总分
`484.2551570027594`。其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为
`18`，libcbench 为 `57.255157002759375`，musl libctest 为 `107`。2026-06-21
12:23 的 9-command lmbench 探针仍为 0，但 basic、BusyBox、Lua 和 libctest
均未回退；总分 `484.15161299502336` 的小幅下降来自 libcbench 性能波动。下一轮
仍只追 lmbench：保留 9 条轻量命令，但加入 `-W 1 -N 10`，让默认 benchmark
尽快输出 `Simple ...`、`Select on ...` 和 `Signal handler ...` 等官方可解析行。
