...
- musl `libctest` 小批量探针已线上验证 8 个稳定 case；当前只识别官方
  ...
  `entry-static.exe` 和可选 `runtest.exe`。当前 12 个 case 已线上通过，本轮从
  12 小步扩到 16 个，只新增 `string_memmem`、`string_strcspn`、`strtol`、
  `strverscmp`。
...
最近可见高分结果为 2026-06-20 16:16:33：编译状态 `Accepted`，总分
`389.00362218124934`。其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为
`18`，libcbench 为 `57.00362218124933`，musl libctest 为 `12`。本轮继续保持
单指标策略，不扩大 lmbench、iozone、ltp、网络或 LoongArch，只把 musl libctest
从 12 个稳定 case 小步扩到 16 个 case。
