...
- lmbench 保留 9-command lite 探针，并补齐官方脚本会准备的运行环境骨架：
  `/bin/sh`、`/lib`、`/lib64/lp64d`、`/usr/lib64`、`/etc/passwd`、
  `/tmp/memfd`、`/var/tmp`、`/lmbench_all`、`lat_sig`、`/var/tmp/XXX`
  和 `/tmp/hello` 等兼容资源；submit 构建默认关闭 `stack_trace`，减少
  syscall/文件热路径的调试栈记录开销。
...
最新可见结果为 2026-06-22 18:46:51：编译状态 `Accepted`，总分
`484.1693353980349`。其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为
`18`，libcbench 为 `57.16933539803484`，musl libctest 为 `107`，lmbench 仍为
`0`。这说明关闭默认 `stack_trace` 没有破坏 483-484 基线，但 24-command
lmbench 主项队列仍未进分，并显著拉长评测时间。
...
下一轮重点验证官方运行环境骨架和 9-command lmbench lite：若总分低于 480，优先
撤回环境骨架；若保持基线且 lmbench 有分，再继续优化 pipe、select 和 fork/exec
热路径。
