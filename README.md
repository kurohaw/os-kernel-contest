...
- 本轮新增 musl `libctest` 小批量探针：只识别官方
  `/musl/libctest_testcode.sh`，读取 `run-static.sh`，暂存
  `entry-static.exe` 和可选 `runtest.exe`，先执行 `argv`、`basename`、`dirname`、
  `env`、`qsort`、`random`、`snprintf`、`string` 这 8 个 allowlist case。
...
最近可见高分结果为 2026-06-20 10:52:03：编译状态 `Accepted`，总分
`377.42523152095464`。其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为
`18`，libcbench 为 `57.42523152095458`。随后 2026-06-20 11:12:19 的
`/proc/self/exe` 真实路径尝试让 libcbench 回退到 `6.0`、总分 `326.0`，该改动已
回退。本轮不再同时改 lmbench、readlinkat、argv 或资源路径，只尝试让 musl
libctest 从 0 开始进分。
