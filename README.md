...
- musl `libctest` 小批量探针已线上验证 8 个稳定 case；当前只识别官方
  `/musl/libctest_testcode.sh`，读取 `run-static.sh`，暂存
  `entry-static.exe` 和可选 `runtest.exe`。本轮从 8 个稳定 case 小步扩到 12 个，
  只新增 `string_memcpy`、`string_memset`、`string_strchr`、`string_strstr`。
...
最近可见高分结果为 2026-06-20 14:43:29：编译状态 `Accepted`，总分
`384.8411392883504`。其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为
`18`，libcbench 为 `56.84113928835043`，musl libctest 为 `8`。本轮继续保持
单指标策略，不扩大 lmbench、iozone、ltp、网络或 LoongArch，只把 musl libctest
从 8 个稳定 case 小步扩到 12 个 case。
