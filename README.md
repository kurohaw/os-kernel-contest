...
最新可见结果为 2026-06-21 13:04:01：编译状态 `Accepted`，总分 `320.0`。
该结果来自 `8690e03` 的 iozone-lite 尝试，basic、BusyBox、Lua 保持得分，但
libcbench 和 libctest 均回到 0；当前已通过 revert 撤回该接入。

当前稳定基线为 2026-06-21 12:36:14：编译状态 `Accepted`，总分
`484.04145452769785`。其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为
`18`，libcbench 为 `57.04145452769787`，musl libctest 为 `107`。最近最高可见
结果为 2026-06-21 12:05:08，总分 `484.2551570027594`，差异主要来自 libcbench
性能波动。

下一轮先确认 revert 后恢复 484 基线；在没有完整官方串口日志前，不再新增或暂存
iozone。后续提分改从现有稳定组暴露的小 syscall/VFS 语义缺口入手。
