...
`18`，libcbench 为 `57.255157002759375`，musl libctest 为 `107`。本轮已完成
musl static libctest 全量得分，下一阶段不再扩大 libctest，优先锁定 484 基线
并转向 lmbench-lite 的单指标小步探针：本轮只把 lmbench 单命令超时从 5 秒放宽到
20 秒，并在原有 6 条 `lat_syscall` 后新增 `lat_select file`、`lat_sig install`
和 `lat_sig catch` 三条官方脚本中的轻量命令。
