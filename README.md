...
glibc、musl basic、BusyBox、Lua、libcbench、musl libctest 和首批 LTP；根构建
同时生成真实 LoongArch `kernel-la`，不再复制 RISC-V 占位 ELF。

## 2026-06-25 双架构里程碑

- `make all` 同时生成 RISC-V `kernel-rv` 和 LoongArch `kernel-la`。
...
- 生成官方要求的 RISC-V `kernel-rv` 和 LoongArch `kernel-la`。
