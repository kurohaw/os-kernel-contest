...
- lmbench 保留 9-command lite 探针，并保留 `/lmbench_all`、`lat_sig`、
  ...
  当前已撤回。
- 当前新增一轮 aggressive 探针：在所有既有稳定组之后，按官方脚本整体尝试
  `cyclictest`、`iperf`、`netperf` 的 glibc/musl 组。该路径只在测试盘存在对应
  `*_testcode.sh` 时启用，每个脚本 20 秒超时；若线上回退，优先直接撤回。
...
最新可见结果为 2026-06-23 15:42:29：编译状态 `Accepted`，总分 `320.0`。
其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为 `18`，但 libcbench 和
libctest 均回到 `0`。这说明 `c5ee433` 的 LTP 12-case 探针会破坏后续稳定计分，
当前已通过 `f7abddf` 撤回。

本轮用户要求更激进冲分，因此在 `f7abddf` 撤回版基础上新增
`cyclictest/iperf/netperf` 脚本级探针。该提交风险高于 484 稳定基线；若总分继续
低于 480，应立即撤回本轮 aggressive 探针。

上一条可信稳定结果为 2026-06-22 18:46:51：编译状态 `Accepted`，总分
`484.1693353980349`。其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为
`18`，libcbench 为 `57.16933539803484`，musl libctest 为 `107`，lmbench 为
`0`。
...
下一轮重点看 aggressive 探针是否能打开 `cyclictest`、`iperf` 或 `netperf` 任一
新分项；若仍为 320 或低于 480，立即撤回该探针，恢复 484 基线。
