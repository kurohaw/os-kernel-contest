...
- musl `libctest` 小批量探针已线上验证 36 个稳定 case；当前只识别官方
  `/musl/libctest_testcode.sh`，读取 `run-static.sh`，暂存
  `entry-static.exe` 和可选 `runtest.exe`。当前 36 个 case 已线上通过，本轮直接
  扩到官方 `static.txt` 的 107 个 case，目标是一次拿满 musl static libctest。
...
最近可见高分结果为 2026-06-21 11:49:38：编译状态 `Accepted`，总分
`412.92336789756513`。其中 RISC-V basic 为 `204`，BusyBox 为 `98`，Lua 为
`18`，libcbench 为 `56.92336789756515`，musl libctest 为 `36`。本轮继续保持
单指标策略，不扩大 lmbench、iozone、ltp、网络或 LoongArch，只把 musl libctest
从 36 个稳定 case 扩到 107 个 static case。
