...
- 当前已能识别官方风格挂载的 virtio-blk 测试盘，从无分区 EXT4 扫描并读取 `*_testcode.sh`，输出官方测试组 START/END 标记；能跳过 `busybox echo`、处理 `cd`、读取嵌套 `.sh`，从脚本中定位第一个真实 ELF 并传入 argv；外部 ELF 支持最小 `argc/argv/envp/auxv` 启动栈、EXT4 普通文件读取（含子目录路径）和 `brk` 增长映射真实用户堆页。
...
- ELF loader 目前只支持把整个 ELF 读入 4 MiB 内核缓冲并映射 `PT_LOAD` 段；已构造脚本命令 argv、空 `envp` 和基础 `auxv`，但尚未支持动态链接器、解释器路径或多程序串行执行。
...
2. 把脚本执行器从“定位第一个 ELF”扩展为“多个 ELF 队列串行运行”。
