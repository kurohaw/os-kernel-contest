...
- 当前已能识别官方风格挂载的 virtio-blk 测试盘，从无分区 EXT4 根目录扫描并读取 `*_testcode.sh`，输出官方测试组 START/END 标记，并从测试盘读取/加载一个 RISC-V ELF 进入用户态，构造最小 `argc/argv/envp/auxv` 启动栈，并让外部 ELF 通过 `openat/read/fstat` 读取 EXT4 根目录普通文件。
...
2. 补真实 `brk` 堆页映射，减少 libc 初始化阶段页错误。
3. 补 per-process fd table 和更完整路径解析，支持脚本/程序访问子目录文件。
